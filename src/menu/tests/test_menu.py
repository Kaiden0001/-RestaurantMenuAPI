...
async def test_get_menu_detail(client: AsyncClient,
                               menu_data: dict,
                               menu_id: str) -> None:
...
async def test_patch_menu(client: AsyncClient,
                          ...
                          menu_id: str) -> None:
    response: Response = await client.patch(f'/menus/{menu_id}',
                                            json=menu_update_data)
    response_json: dict = response.json()
...
async def test_patch_menu_invalid_id(client: AsyncClient,
                                     menu_update_data: dict) -> None:
    response: Response = await client.patch(f'/menus/{uuid.uuid4()}',
                                            json=menu_update_data)
